...
function DynamicInstructorSheet_createDynamicInstructorSheet() {
  if (ErrorHandling && typeof ErrorHandling.logMessage === 'function') {
    ErrorHandling.logMessage('Menu: Generate Group Lesson Tracker', 'INFO', 'DynamicInstructorSheet_createDynamicInstructorSheet');
  }
  return DynamicInstructorSheet.createDynamicInstructorSheet();
}

// The updateWithSelectedClass menu function has been removed since this functionality 
// is now handled automatically when selecting a class in the dropdown
...
/**
 * Menu wrapper for syncing student data between Group Lesson Tracker and SwimmerSkills
 */
function syncSwimmerData_menuWrapper() {
  if (ErrorHandling && typeof ErrorHandling.logMessage === 'function') {
    ErrorHandling.logMessage('Menu: Sync Student Data', 'INFO', 'syncSwimmerData_menuWrapper');
  }
  return syncSwimmerData();
}