 ...
 * @param source - Source of the message (function or module name)
 ...
 * @param source - Source of the error (function or module name)
 ...
 * @param includeCancel - Whether to include a Cancel button
...
function confirmAction(title, message, includeCancel = true) {
  try {
    ...
    const result = ui.alert(
      title,
      message,
      includeCancel ? ui.ButtonSet.YES_NO_CANCEL : ui.ButtonSet.YES_NO
    );
 ...
 * @return URL of the exported spreadsheet or empty string on failure
 */
function exportLog() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
