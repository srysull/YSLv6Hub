 ...
 * 
 * @returns The created menu or null if creation failed
 */
function createMenu() {
  try {
    ...
    const hasStartedInit = Boolean(sessionName) && !isInitialized;
    ...
    if (!isInitialized && !forceOperationalMenu) {
      ...
      try {
        ...
        menu.addSubMenu(ui.createMenu('Class Management')
              .addItem('Generate Group Lesson Tracker', 'DynamicInstructorSheet_createDynamicInstructorSheet')
              .addSeparator()
 ...
 * @param reportTemplateUrl - URL of the report template folder
 * @param swimmerRecordsUrl - URL of the swimmer records workbook
 * @param sessionProgramsUrl - URL of the session programs workbook
 * @param parentHandbookUrl - URL of the parent handbook PDF (optional)
 */
function prepareAssumptionsSheetWithValues(
  sessionName, 
  reportTemplateUrl, 
  swimmerRecordsUrl, 
  sessionProgramsUrl, 
...
/**
 * Completes the full system initialization using values from the Assumptions sheet
 */
  ...
  try {
    ...
    Object.entries(validatedValues).forEach(([key, value]) => {
      if (value) { // Only store non-empty values
        ...
        if (configKey) {
          GlobalFunctions.safeSetProperty(configKey, value);
        }
      }
    });
...
function showConfigurationDialog() {
  try {
    ...
    try {
      // Make sure assumptionsSheet is not null before using it
      if (assumptionsSheet) {
      ...
      Object.entries(configValues).forEach(([key, value]) => {
        ...
        if (propKey) {
          ...
          if (value && value !== currentValue) {
            GlobalFunctions.safeSetProperty(propKey, value.toString());
            changesMade = true;
...
const AdministrativeModule = {
  ...
  prepareAssumptionsSheet,
  prepareAssumptionsSheetWithValues,
  ...
  fixSystemInitializationProperty
};